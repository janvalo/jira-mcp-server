...
export class JiraClient {
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `✅ Successfully created Jira task: ${issue.key}\n\n**Summary:** ${args.summary}\n**Project:** ${args.projectKey}\n**Type:** ${args.issueType || "Task"}\n**Priority:** ${args.priority || "Medium"}\n\nView at: ${this.client.defaults.baseURL}/browse/${issue.key}`,
                    },
                ],
            };
        }
        catch (error) {
            throw new Error(`Failed to create Jira task: ${this.getErrorMessage(error)}`);
        }
    }
    async updateTaskStatus(args) {
        try {
            // First, get the available transitions for this issue
            const transitionsResponse = await this.client.get(`/rest/api/3/issue/${args.issueKey}/transitions`);
            const transitions = transitionsResponse.data.transitions;
            // Find the transition that matches the target status
            const targetTransition = transitions.find((t) => t.to.name.toLowerCase() === args.status.toLowerCase());
            if (!targetTransition) {
                const availableStatuses = transitions
                    .map((t) => t.to.name)
                    .join(", ");
                throw new Error(`Status "${args.status}" not available. Available statuses: ${availableStatuses}`);
            }
            ...
            if (workInProgressStatuses.includes(args.status.toLowerCase())) {
                ...
                const issueResponse = await this.client.get(`/rest/api/3/issue/${args.issueKey}`);
            ...
            await this.client.post(`/rest/api/3/issue/${args.issueKey}/transitions`, transitionPayload);
            // Auto-assign if moving to "In Progress" and not already assigned
            if (shouldAutoAssign) {
                try {
                    // Get the current user's accountId for assignment
                    const userResponse = await this.client.get(`/rest/api/3/user/search?query=${process.env.JIRA_EMAIL}`);
                    const user = userResponse.data[0];
                    if (user && user.accountId) {
                        const assignPayload = {
                            accountId: user.accountId,
                        };
                        await this.client.put(`/rest/api/3/issue/${args.issueKey}/assignee`, assignPayload);
                    }
                }
                catch (assignError) {
            ...
            throw new Error(`Failed to update task status: ${this.getErrorMessage(error)}`);
        }
    }
    async updateTaskProgress(args) {
        try {
                    ...
                    content: [
                        {
                            ...
                            content: [
                                {
                                    type: "text",
                                    text: `Progress updated to ${args.progressPercent}%${args.comment ? ` - ${args.comment}` : ""}`,
                                },
                            ],
                        },
                    ],
            ...
            await this.client.post(`/rest/api/3/issue/${args.issueKey}/comment`, commentPayload);
            return {
                content: [
                    {
                        type: "text",
                        text: `✅ Successfully updated progress of ${args.issueKey} to ${args.progressPercent}%${args.comment ? `\n\n**Comment:** ${args.comment}` : ""}\n\nView at: ${this.client.defaults.baseURL}/browse/${args.issueKey}`,
                    },
                ],
            };
        }
        catch (error) {
            throw new Error(`Failed to update task progress: ${this.getErrorMessage(error)}`);
        }
    }
    async getTask(args) {
        try {
            const response = await this.client.get(`/rest/api/3/issue/${args.issueKey}`);
            const issue = response.data;
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Found ${issues.length} task(s):\n\n${taskList}\n\nView all at: ${this.client.defaults.baseURL}/issues/?jql=${encodeURIComponent(jql)}`,
                    },
                ],
            };
        }
